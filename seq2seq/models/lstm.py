...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        2.  Why do we need to apply a mask to the attention scores?
            The mask is applied to the attention scores to deal with sentences of different lengths.
            Shorter sentences are padded with -inf so that when softmax is applied the attn_weights for these are 0.
            This makes the attention mechanism ignore these tokens that were padded with -inf.
        '''
        ...
        # attn_weights.size = [batch_size, 1, src_time_steps]     same as attn_scores.size
        attn_context = torch.bmm(attn_weights, encoder_out).squeeze(dim=1)
        # torch.bmm(attn_weights, encoder_out).size = [batch_size, 1, output_dims]
    ...
    def score(self, tgt_input, encoder_out):
        ...
        2.  How are attention scores calculated?
            The encoder output is projected using a linear layer to the dimensions of output_dims.
            This is then transposed so that the dimensions match up with dimensions needed to work with tgt_input.
            The tgt_input is then unsqueezed and batch matrix multiplication of the unsqueezed tgt_input
            and the projected encoder output is performed to obtain the attn score.
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            self.source_embed = nn.Linear(self.embed_dim, self.embed_dim, bias = False)
            self.lexical_output = nn.Linear(self.embed_dim, len(dictionary))
            # TODO: --------------------------------------------------------------------- /CUT
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        1.  When is cached_state == None?
            Cached state is None when nothing is fed into the incremental_state parameter, which happens when the model isn't run on incremental mode.
            It is also none when the key 'cached_state' is not in the incremental_state dictionary, which should happen at the start of the model run.


        2.  What role does input_feed play?
            input_feed is the output from the previous time step and is concatenated to the current token embedding. This is then used as
            LSTM input for the current timestep. If attention is used, input_feed is attended to before being used as input to the LSTM
            for the current iteration.
            (This lets information from previous states to be used, allowing dependencies between tokens to be captured)
        '''
        ...
        for j in range(tgt_time_steps):
            ...
            1.  Why is the attention function given the previous target state as one of its inputs?
                The previous target state is given to the attention function to calculate the attention weights for the current target state.
                This is due to how attention is defined, its calculation depends on the previous state.

            2.  What is the purpose of the dropout layer?
                The dropout layer helps to prevent overfitting and over-parameterization of the model.
                By setting layer units to 0 at random, the model needs to learn more robust features
                and is more likely to generalise well to the test data.
            '''
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    # TODO: --------------------------------------------------------------------- CUT
                    # f^l_t = tanh(sum_s(a_t(s) f_s))
                    weighted_average_sum = torch.bmm(step_attn_weights.unsqueeze(1),
                                                  src_embeddings.transpose(0, 1))
                    average_source_word_embedding = torch.tanh(weighted_average_sum.squeeze(1))

                    # h^l_t = tanh(W f^l_t) + f^l_t
                    h_lexical = torch.tanh(self.source_embed(average_source_word_embedding) + average_source_word_embedding)
                    lexical_contexts.append(h_lexical)
                    # TODO: --------------------------------------------------------------------- /CUT
        ...
        if self.use_lexical_model:
            ...
            decoder_output = decoder_output + self.lexical_output(lexical_contexts)

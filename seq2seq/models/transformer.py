    ...
    def forward(self, tgt_inputs, encoder_out=None, incremental_state=None, features_only=False):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            3.  Why do we need it in the decoder but not in the encoder?
                The mask is needed in the decoder because its supposed to generate the output tokens sequentialy, so during
                inference it won't have generated future tokens so wouldn't be able to use them.
                The encoder, on the other hand, is given all the input tokens at once during inference, thus it
                has access to future tokens so a mask isn't needed during training.
        ...
        if not features_only:
            ...
            2.  What would the output represent if features_only=True?
                If features_only=True, the output would be the feature representation
